      ...
      <div className="v2-container">
        <Reveal>
          <SectionHeading
            ...
            title={<>Đội ngũ không cần thêm công cụ. Họ cần <span className="v2-gradient-text">một nhịp làm việc.</span></>}
            ...
            return (
              <Reveal key={shift.before} delay={index * 80}>
                <article className="v2-shift-card h-full">
                  <div className={`v2-icon-box v2-icon-box--${shift.color}`}><Icon className="size-5" /></div>
                  <div className="mt-7 flex items-center gap-2 text-sm text-[var(--v2-muted)]">
                    <span className="line-through decoration-[var(--v2-coral)]/60">{shift.before}</span>
                    <ArrowRight className="size-3.5" />
                  </div>
                  <h3 className="mt-2 text-xl font-semibold tracking-[-0.035em] text-[var(--v2-ink)]">{shift.after}</h3>
                  <p className="mt-3 text-sm leading-6 text-[var(--v2-muted)]">{shift.text}</p>
                </article>
              </Reveal>
            )
          })}
