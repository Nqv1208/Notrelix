...
import { requireWorkspaceId, requireWorkspaceMembership } from '../router/guards/require-workspace-membership';

describe('route guard utilities', () => {
  it('validates workspace route params before component render', () => {
    expect(requireWorkspaceId({ workspaceId: 'workspace_1' })).toBe('workspace_1');
  ...
  it('throws typed guard errors for missing membership, permissions, entitlements and feature flags', () => {
    expect(() =>
      requireWorkspaceMembership({ workspaceId: 'ws-2', memberWorkspaceIds: ['ws-1'] }),
