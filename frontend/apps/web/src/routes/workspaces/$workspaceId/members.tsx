...
export function MembersPage() {
  ...
  const handleInvite = async () => {
    ...
    try {
      await createInvitationMutation.mutateAsync({ email: inviteEmail.trim(), role: inviteRole });
      setInviteEmail('');
    ...
    <div className="p-8 max-w-4xl">
      <div className="mb-8">
        ...
        <p className="text-sm text-muted-foreground">
          Manage who has access to {workspace?.name ?? 'this workspace'} and control their permission levels.
        </p>
      </div>
      ...
      <div className="p-5 rounded-xl border border-border/80 bg-muted/10 mb-8">
        ...
        >
          ...
          <Button type="submit" disabled={createInvitationMutation.isPending || !inviteEmail.trim()} className="rounded-lg px-5">
      ...
      <div className="space-y-3 mb-8">
        <h3 className="text-sm font-semibold text-foreground">Active members ({members.length})</h3>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold text-foreground text-sm flex items-center gap-1.5">
                            {member.name}
                            {member.role === 'owner' && (
                              <Badge variant="outline" className="text-[9px] py-0 px-1.5 font-normal tracking-wide bg-primary/5 border-primary/20 text-primary rounded-md">Owner</Badge>
                            )}
                          </p>
                        </div>
                      </div>
      ...
      {invitations.length > 0 && (
        <div className="space-y-3 pt-4 border-t border-border/40">
          <h3 className="text-sm font-semibold text-foreground">Pending invitations ({invitations.length})</h3>
          <div className="rounded-xl border border-border/60 overflow-hidden bg-card">
            <Table>
              <TableHeader className="bg-muted/30">
                <TableRow className="hover:bg-transparent">
