...
export function WorkspaceSidebar() {
  ...
  const primaryNav: NavItem[] = [
    { label: 'Home', icon: Home, to: '/workspaces/$workspaceId', params: { workspaceId } },
    { label: 'My Work', icon: UserRoundCheck, to: '/workspaces/$workspaceId', params: { workspaceId } },
    { label: 'Inbox', icon: Inbox, to: '/workspaces/$workspaceId', params: { workspaceId } },
    { label: 'Notifications', icon: Bell, to: '/workspaces/$workspaceId', params: { workspaceId } },
    { label: 'Chat Rooms', icon: MessageSquareText, to: '/workspaces/$workspaceId', params: { workspaceId } },
  ];

  const supportNav: NavItem[] = [
    { label: 'Help / Support', icon: LifeBuoy, to: '/workspaces/$workspaceId', params: { workspaceId } },
    { label: 'Settings', icon: Settings, to: '/workspaces/$workspaceId/settings', params: { workspaceId } },
  ];
    ...
    <aside className="w-64 border-r bg-card flex flex-col h-screen text-card-foreground">
      ...
      <ScrollArea className="min-h-0 flex-1 px-3">
        <nav className="space-y-1">
          {primaryNav.map((item) => {
            const isActive = item.to === '/workspaces/$workspaceId'
              ? pathname === `/workspaces/${workspaceId}`
              : pathname.startsWith(item.to.replace('$workspaceId', workspaceId));
            return (
              <Link
                key={item.label}
