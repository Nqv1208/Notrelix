...
function createHarness() {
  ...
  return { queryClient, workspaceEvents, realtime, clearWorkspaceState, lifecycle };
}
...
describe('application lifecycle — workspace isolation', () => {
  it('LIFE-020 full QueryClient cache is empty before B activation completes', () => {
    ...
    workspaceEvents.publish({ previousWorkspaceId: 'ws-a', nextWorkspaceId: 'ws-b' });
    expect(queryClient.getQueryCache().getAll()).toHaveLength(0);
    lifecycle.dispose();
  });

  it('LIFE-021 workspace transient reset is called', () => {
    const { queryClient, workspaceEvents, clearWorkspaceState, lifecycle } = createHarness();
    seedWorkspaceA(queryClient);
  ...
  it('LIFE-022 realtime A connection/subscriptions are disconnected/reset', () => {
    const { queryClient, workspaceEvents, realtime, lifecycle } = createHarness();
    seedWorkspaceA(queryClient);
  ...
  it('LIFE-023 B activation/reconnect happens only after old state reset', () => {
    const { queryClient, workspaceEvents, realtime, clearWorkspaceState, lifecycle } =
      createHarness();
    seedWorkspaceA(queryClient);
  ...
  it('LIFE-024 switching A -> A is a no-op', () => {
    ...
    workspaceEvents.publish({ previousWorkspaceId: 'ws-a', nextWorkspaceId: 'ws-a' });
  ...
  it('LIFE-025 rapid A -> B -> C cannot restore A/B cache state', () => {
    ...
    workspaceEvents.publish({ previousWorkspaceId: 'ws-b', nextWorkspaceId: 'ws-c' });
    expect(queryClient.getQueryCache().getAll()).toHaveLength(0);
