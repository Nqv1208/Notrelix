...
import { useCardFiles } from "@/features/boards/hooks"
import type { CardDetail } from "@/features/boards/types"
...
export function TaskFilesTab({ card }: { card: CardDetail }) {
  ...
  const filteredFiles = useMemo(() => {
    ...
    return data.filter((file) => file.name.toLowerCase().includes(normalized))
  }, [data, query])
    ...
    <div className="flex flex-col gap-4 p-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <ToggleGroup type="single" value={mode} onValueChange={(value) => value && setMode(value as "grid" | "list")}>
