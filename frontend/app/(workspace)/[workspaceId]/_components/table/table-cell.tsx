...
export function TableCell({
  ...
  if (field.fieldType === "select") {
    const value = semantic === "priority" ? card.priority : semantic === "status" ? card.status : String(card.fieldValues[field.id] ?? "")
      ...
      <DropdownMenuTrigger asChild>
        <button type="button" className="max-w-full text-left" onClick={(event) => event.stopPropagation()} aria-label={`${field.name}: ${option?.label ?? "Empty"}`}>
          {option ? (
            <Badge
              variant="secondary"
              className={cn("max-w-full rounded-full border", getOptionToneClass(option.id))}
            >
...
function TextFieldCell({ card, field }: { card: Card; field: FieldDefinition }) {
  ...
  return (
    <button type="button" className="min-w-0 truncate text-left text-sm text-muted-foreground" onClick={(event) => {
      event.stopPropagation()
      setEditing(true)
    }}>
      {value || "Empty"}
