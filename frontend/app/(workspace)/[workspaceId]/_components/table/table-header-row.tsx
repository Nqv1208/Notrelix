...
import { useDeleteColumn, useUpdateColumn } from "@/features/boards/hooks"
import type { BoardTableColumn } from "@/features/boards/types"
...
export function TableHeaderRow({
  ...
  return (
    <div
      role="row"
      className="grid min-h-10 bg-card text-xs font-semibold uppercase text-muted-foreground"
      style={{ gridTemplateColumns: gridTemplate }}
    >
      <div role="columnheader" className="sticky left-0 z-30 flex items-center gap-2 border-r border-border bg-card px-3">
            ...
            {!column.field.id.endsWith("field-title") ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button
                    type="button"
                    className="rounded p-1 text-muted-foreground opacity-0 transition hover:bg-background hover:text-foreground group-hover/header:opacity-100"
