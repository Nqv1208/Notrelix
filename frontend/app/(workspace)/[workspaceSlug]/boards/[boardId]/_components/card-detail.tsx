    ...
    <main className={cn(mode === "page" ? "mx-auto max-w-[1180px] px-4 py-6 sm:px-6 lg:px-8" : "h-[92vh] overflow-auto p-5")}>
      <div className="mb-5 flex items-center justify-between gap-3 rounded-2xl border border-border bg-card p-3">
        <div className="flex min-w-0 items-center gap-2">
          <Button variant="ghost" size="icon-sm" asChild>
            <Link href={`/${workspaceSlug}/boards/${boardId}` as never} aria-label="Back to board">
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
        <section className="space-y-5">
          ...
          {card.linkedPageId ? <CardLinkedPagePreview workspaceSlug={workspaceSlug} boardId={boardId} pageId={card.linkedPageId} /> : (
        ...
        <CardSidebar card={card} workspaceSlug={workspaceSlug} boardId={boardId} />
      </div>
    </main>
  )
}
