...
import type { DocsWorkspaceSnapshot, Page, PageTreeNode } from "@/features/docs/types"
    ...
    <div className="flex h-svh flex-col bg-card">
      <div className="border-b border-border p-4">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <NewPageButton workspaceId={workspace.id} workspaceSlug={workspaceSlug} compact />
        </div>
        <DocsSearch workspaceId={workspace.id} workspaceSlug={workspaceSlug} mode="inline" />
      </div>
          ...
          {isLoading ? (
            ...
            <>
              <SidebarSection title="Favorites" icon={Star}>
                <CompactPageList pages={favorites} workspaceSlug={workspaceSlug} empty="No favorites yet" />
              </SidebarSection>
              <SidebarSection title="Recent" icon={Clock3}>
                <CompactPageList pages={recentPages} workspaceSlug={workspaceSlug} empty="No recent pages" />
              </SidebarSection>
              <SidebarSection title="Workspace tree" icon={Layers3} action={<Button variant="ghost" size="icon-xs"><Plus className="size-3" /></Button>}>
                {pageTree.length ? (
                  <PageTree tree={pageTree} workspaceSlug={workspaceSlug} density="compact" />
                ) : (
                  <p className="px-2 py-3 text-xs text-muted-foreground">No pages yet</p>
                )}
              </SidebarSection>
              <SidebarSection title="Shared" icon={Share2}>
                <CompactPageList pages={sharedPages} workspaceSlug={workspaceSlug} empty="No shared pages" />
              </SidebarSection>
            </>
          )}
        </div>
...
function CompactPageList({ pages, workspaceSlug, empty }: { pages: Page[]; workspaceSlug: string; empty: string }) {
