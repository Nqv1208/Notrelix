...
export const PageTree = memo(function PageTree({ tree, workspaceSlug, density = "default" }: PageTreeProps) {
  return (
    <nav aria-label="Page tree" className="space-y-0.5">
      {tree.map((node) => (
        <PageTreeItem key={node.id} node={node} workspaceSlug={workspaceSlug} density={density} />
      ))}
    </nav>
  )
})
...
function PageTreeItem({
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      <ContextMenu>
        <ContextMenuTrigger asChild>
          ...
          >
            ...
            <Link href={`/${workspaceSlug}/docs/${node.id}`} className="flex min-w-0 flex-1 items-center gap-2">
      ...
      {hasChildren ? (
        <CollapsibleContent>
          <PageTree tree={node.children} workspaceSlug={workspaceSlug} density={density} />
        </CollapsibleContent>
      ) : null}
    </Collapsible>
  )
