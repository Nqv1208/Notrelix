...
export function WorkspaceSidebar({ workspaceSlug, collapsed, onCollapse, onExpand, inSheet }: WorkspaceSidebarProps) {
  ...
  const primaryNav: NavItem[] = [
    { label: "Home", icon: Home, href: `/${workspaceSlug}` },
    { label: "My Work", icon: UserRoundCheck, href: `/${workspaceSlug}?panel=my-work`, panel: "my-work" },
    { label: "Inbox", icon: Inbox, href: `/${workspaceSlug}?panel=inbox`, panel: "inbox" },
    { label: "Chat Rooms", icon: MessageSquareText, href: `/${workspaceSlug}/chat`, path: `/${workspaceSlug}/chat` },
  ]
  const managementNav: NavItem[] = [
    { label: "Members", icon: Users, href: `/${workspaceSlug}?panel=members`, panel: "members" },
    { label: "Settings", icon: Settings, href: `/${workspaceSlug}?panel=settings`, panel: "settings" },
    { label: "Permissions", icon: LockKeyhole, href: `/${workspaceSlug}?panel=permissions`, panel: "permissions" },
    { label: "Integrations", icon: Sparkles, href: `/${workspaceSlug}?panel=integrations`, panel: "integrations" },
    { label: "Automations", icon: Workflow, href: `/${workspaceSlug}?panel=automations`, panel: "automations" },
    { label: "Activity Logs", icon: Activity, href: `/${workspaceSlug}?panel=activity`, panel: "activity" },
  ]
  const supportNav: NavItem[] = [
    { label: "Notifications", icon: Bell, href: `/${workspaceSlug}?panel=notifications`, panel: "notifications" },
    { label: "Help / Support", icon: LifeBuoy, href: `/${workspaceSlug}?panel=support`, panel: "support" },
  ]
          ...
          <nav className="space-y-1">
            {primaryNav.map((item) => (
              <SidebarNavLink
                ...
                item={item}
                collapsed={collapsed}
                active={isActiveWorkspaceNav({ item, pathname, panel, workspaceSlug })}
              />
            ))}
          </nav>
...
function isActiveWorkspaceNav({
  ...
  if (item.panel) return pathname === `/${workspaceSlug}` && panel === item.panel
