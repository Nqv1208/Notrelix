...
import { useFullBoard } from "@/features/boards/hooks"
...
export function WorkspaceViewContent({
  ...
  if (view.type === "kanban") return <WorkspaceBoardView mode="kanban" view={view} />
  if (view.type === "doc") return <WorkspaceDocView pageId={view.target.pageId ?? "docs-mvp-spec"} workspaceSlug={workspaceSlug} />
  if (view.type === "calendar") return <WorkspaceCalendarView view={view} />
  if (view.type === "timeline") return <WorkspaceTimelineView view={view} />
  if (view.type === "dashboard") return <WorkspaceDashboardView workspaceSlug={workspaceSlug} snapshot={snapshot} />
  if (view.type === "table") return <WorkspaceBoardView mode="table" view={view} />
  return <UnsupportedView view={view} />
}

function WorkspaceBoardView({ mode, view }: { mode: "table" | "kanban"; view: WorkspaceView }) {
  const boardId = view.target.boardId ?? "board-product"
  const { board, groups, fieldDefinitions, isLoading, error } = useFullBoard(boardId)

  if (isLoading) return <ViewSkeleton rows={mode === "table" ? 8 : 4} />
...
function WorkspaceDocView({ pageId, workspaceSlug }: { pageId: string; workspaceSlug: string }) {
  return (
    <div className="h-full min-h-[720px] bg-background">
      <MondayDocEditor pageId={pageId} workspaceSlug={workspaceSlug} embedded showToolbar={false} showOpenFullDoc />
    </div>
  )
}

function WorkspaceCalendarView({ view }: { view: WorkspaceView }) {
  const { groups, isLoading, error } = useFullBoard(view.target.boardId ?? "board-product")
  if (isLoading) return <ViewSkeleton rows={6} />
  if (error) return <ViewError title="Calendar unavailable" />
...
function WorkspaceTimelineView({ view }: { view: WorkspaceView }) {
  const { groups, isLoading, error } = useFullBoard(view.target.boardId ?? "board-product")
  if (isLoading) return <ViewSkeleton rows={7} />
  if (error) return <ViewError title="Timeline unavailable" />
...
function WorkspaceDashboardView({ workspaceSlug, snapshot }: { workspaceSlug: string; snapshot: WorkspaceSnapshot }) {
  const metrics = [
        ...
        <section className="rounded-2xl border border-border bg-card p-5">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Button asChild variant="outline" size="sm" className="bg-card">
              <Link href={`/${workspaceSlug}?view=table` as never}>Open table</Link>
            </Button>
          </div>
