...
export function WorkspaceBoardShell({
  ...
  const snapshot = useWorkspaceSnapshot(workspaceSlug)
  const { views, activeView, isLoading, error } = useActiveWorkspaceView(workspaceSlug, requestedView)
  ...
  return (
    <main className="flex h-full min-h-0 flex-col bg-background">
      ...
      <WorkspaceViewTabs workspaceSlug={workspaceSlug} views={views} activeViewId={activeView.id} />
      <WorkspaceContextualToolbar activeType={activeView.type} activeView={activeView} />
      <section className="min-h-0 flex-1 overflow-auto">
        <WorkspaceViewContent workspaceSlug={workspaceSlug} view={activeView} snapshot={snapshot.data} />
      </section>
    </main>
  )
}
