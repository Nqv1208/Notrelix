...
export interface SessionExpiredEvent {
  ...
  readonly reason: "refresh-rejected" | "refresh-network-failure" | "session-revoked"
...
export function createNotrelixClient(config: NotrelixClientConfig) {
  ...
  const createCorrelationId = config.createCorrelationId ?? generateCorrelationId
  ...
  async function refreshOnce(): Promise<void> {
    if (!refreshPromise) {
      refreshPromise = (async () => {
        ...
        try {
          refreshResponse = await fetchImpl(`${customBaseUrl}${endpoints.auth.refresh}`, {
            method: "POST",
            credentials: "include",
            headers: {
