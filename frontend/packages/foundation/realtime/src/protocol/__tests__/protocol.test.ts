...
describe('Realtime Protocol Parsing', () => {
  ...
  it('rejects envelope with invalid timestamp', () => {
    const res = parseRealtimeMessage({ ...validEnvelope, timestamp: 'invalid-date' });
    expect(res.ok).toBe(false);
    if (!res.ok) {
      expect(res.error.reason).toBe('invalid-envelope');
    }
  ...
  it('parses control message: ping', () => {
    const res = parseRealtimeMessage({ type: 'ping', sentAt: new Date().toISOString() });
    expect(res.ok).toBe(true);
  ...
  it('parses control message: pong', () => {
    const res = parseRealtimeMessage({ type: 'pong', sentAt: new Date().toISOString() });
    expect(res.ok).toBe(true);
    if (res.ok && res.value.kind === 'control') {
  ...
  it('parses control message: subscribed', () => {
    const res = parseRealtimeMessage({ type: 'subscribed', subscriptionId: 'sub-1' });
    expect(res.ok).toBe(true);
    if (res.ok && res.value.kind === 'control') {
  ...
  it('parses control message: subscription-error', () => {
    const res = parseRealtimeMessage({ type: 'subscription-error', code: 'unauthorized' });
    expect(res.ok).toBe(true);
    if (res.ok && res.value.kind === 'control') {
  ...
  it('parses JSON string envelope and preserves payload', () => {
    ...
    expect(res.ok).toBe(true);
    if (res.ok && res.value.kind === 'domain') {
      expect(res.value.envelope.payload).toEqual({ id: 'item-1', status: 'done' });
    } else {
      throw new Error('Expected ok domain result');
    }
  });
