...
describe('RealtimeClient foundation contract', () => {
  ...
  async function connectAndOpen(client: RealtimeClient): Promise<WebSocketLike> {
  ...
  it('RT-004 valid incoming message is parsed and dispatched through the subscription API', async () => {
    ...
    expect(listener.mock.calls[0]?.[0]).toMatchObject({ eventId: 'evt-1', workspaceId: 'ws-1' });
  });
  ...
  it('RT-005 malformed message calls the error port and does not crash the message loop', async () => {
    ...
    const client = new RealtimeClient(TEST_URL, { socketFactory: factory, telemetry });
    const listener = vi.fn();
    client.subscribe({ workspaceId: 'ws-1' }, listener);
  ...
  it('RT-006 network close schedules exactly one reconnect', async () => {
    ...
    const client = new RealtimeClient(TEST_URL, { socketFactory: factory, scheduler });
  ...
  it('RT-007 reconnect delay follows deterministic 1s,2s,4s,... capped at 30s', async () => {
    const { scheduler, activeTimeouts, fireLastTimeout } = createFakeScheduler();
    const client = new RealtimeClient(TEST_URL, { socketFactory: factory, scheduler });
    ...
    expect(reconnectDelays).toEqual([1000, 2000, 4000, 8000, 16000, 30000, 30000]);
  });
  ...
  it('RT-013 subscriptions survive network reconnect', async () => {
    ...
    socketAt(1).onmessage?.({ data: validEnvelope({ eventId: 'evt-after-reconnect' }) });
    expect(listener).toHaveBeenCalledTimes(1);
  ...
  it('no exported global realtime singleton exists in production source', () => {
    ...
    for (const file of sources) {
      const content = readFileSync(file, 'utf8');
      expect(content, file).not.toMatch(/export\s+(const|let|var)\s+realtimeClient\b/);
      expect(content, file).not.toMatch(/export\s+(const|let|var)\s+\w*[Rr]ealtime\w*\s*=\s*new\s+RealtimeClient/);
    }
  });
});
