...
function validEnvelope(eventId: string, overrides: Record<string, unknown> = {}): string {
...
describe('integration: composition root wires kernel, contracts, and realtime', () => {
  it('createAppRuntime builds a client from the parsed kernel env', () => {
    ...
    expect(runtime.api).toBeInstanceOf(createNotrelixClient({ baseUrl: 'https://x' }).constructor);
  ...
  it('realtime transport delivers a protocol envelope to a listener end-to-end', async () => {
    ...
    const client = new RealtimeClient('wss://ws.test/realtime', { socketFactory: factory });
