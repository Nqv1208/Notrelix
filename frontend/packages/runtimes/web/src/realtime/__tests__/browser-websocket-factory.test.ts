...
const realtimeConstructorCalls: Array<{ url: string; options: Record<string, unknown> }> = [];
...
describe('browser realtime socket factory', () => {
  it('RT-020 constructs the browser WebSocket with the exact URL', () => {
    ...
    expect((socket as unknown as FakeBrowserWebSocket).url).toBe('wss://realtime.example/realtime');
  });
...
describe('app runtime realtime composition', () => {
  ...
  it('RT-022 routes the realtime error callback to the telemetry port', () => {
    ...
    realtimeTelemetry.reportError(new Error('socket failed'), { context: 'socketError' });
