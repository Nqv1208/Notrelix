...
export function useBoardTable(boardId: string, workspaceId: string) {
  ...
  const {
    ...
    collapsedGroups,
    toggleGroup,
  } = useBoardGroups(fullBoard.groups, viewConfig, updateViewConfig);
  const { query, setQuery, debouncedQuery, filteredGroups } =
    useTableSearch(allGroups);
  ...
  const cards = useMemo(
    () => groups.flatMap((group: any) => group.cards),
    [groups],
  );
  ...
  const toggleAllSelection = useCallback(() => {
    setSelectedCardIds((current) =>
      current.length === cards.length ? [] : cards.map((card: any) => card.id),
    );
  }, [cards]);
  ...
  return {
    ...
    selectionState: {
      selectedCardIds,
      isAllSelected:
        cards.length > 0 && selectedCardIds.length === cards.length,
    },
    toggleCardSelection,
    setCardSelected,
    toggleAllSelection,
