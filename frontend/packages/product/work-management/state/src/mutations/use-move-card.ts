...
export function useMoveCard(boardId: string, workspaceId?: string) {
  ...
  return useMutation({
    mutationFn: (payload: MoveCardInput) => executeOptimisticCommand({
          ...
          (old, variables) => {
            ...
            return {
              ...
              groups: old.groups.map((group) => {
                const withoutCard = group.cards.filter((card) => card.id !== variables.cardId)
                if (group.id !== variables.listId) return { ...group, cards: withoutCard }
                return {
                  ...group,
      ...
      mutationFn: (variables, context) => cards.moveCard(variables, {
