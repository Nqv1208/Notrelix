...
export function useUpdateFieldValue(boardId: string, workspaceId?: string) {
  ...
  return useMutation({
    mutationFn: (payload: UpdateFieldValueInput) =>
      cards.updateFieldValue(payload),
    onMutate: async (payload) => {
      ...
      queryClient.setQueryData<FullBoardResponse>(queryKey, (old) =>
        updateCardInFullBoard(old, payload.cardId, (card: Card) => {
          const memberIds = Array.isArray(payload.value)
          ...
          const fieldDef = previous?.fieldDefinitions.find(
            (f) => f.id === payload.fieldDefinitionId,
          );
          const isDueDateField =
            fieldDef?.fieldType === "date" ||
            payload.fieldDefinitionId.endsWith("field-due-date");
          ...
          return {
            ...
            status:
              ...
              typeof payload.value === "string"
                ? payload.value
                : card.status,
            priority: payload.fieldDefinitionId.endsWith("field-priority")
              ? (payload.value as Card["priority"])
              : card.priority,
            dueDate: isDueDateField
              ? ((payload.value as string | null) ?? undefined)
              : card.dueDate,
            members,
            fieldValues: {
              ...card.fieldValues,
              [payload.fieldDefinitionId]: payload.value,
            },
