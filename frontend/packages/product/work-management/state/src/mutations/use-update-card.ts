        ...
        updateCardInFullBoard(old, cardId, (card: Card) => {
          const next = {
            ...
            priority:
              patch.priority === null
                ? undefined
                : (patch.priority ?? card.priority),
            dueDate:
              patch.dueDate === null
                ? undefined
                : (patch.dueDate ?? card.dueDate),
            startDate:
              patch.startDate === null
                ? undefined
                : (patch.startDate ?? card.startDate),
            fieldValues: {
              ...card.fieldValues,
              [`${card.boardId}-field-title`]:
                patch.title ?? card.fieldValues[`${card.boardId}-field-title`],
              [`${card.boardId}-field-due-date`]:
                patch.dueDate ??
                card.fieldValues[`${card.boardId}-field-due-date`],
              [`${card.boardId}-field-priority`]:
                patch.priority ??
                card.fieldValues[`${card.boardId}-field-priority`],
            },
