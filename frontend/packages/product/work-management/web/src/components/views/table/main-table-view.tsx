...
export function MainTableView({
  ...
  const gridTemplate = useMemo(
    () => getMainTableGridTemplate(table.columns),
    [table.columns],
  );
            ...
            <div className="min-h-0 flex-1">
              ...
              >
                <TableScrollContainer>
                  <div
                    role="grid"
                    aria-rowcount={visibleGroups.reduce(
    ...
    <div className="flex min-h-14 shrink-0 flex-wrap items-center gap-2 bg-card px-4 py-2 sm:px-6">
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="start" className="w-52">
          ...
          <DropdownMenuItem onClick={() => table.tableFilters.clearFilters()}>
            Clear filters
          </DropdownMenuItem>
          <DropdownMenuSeparator />
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="start" className="w-52">
          <DropdownMenuItem onClick={() => table.tableSort.clearSort()}>
            Clear sort
          </DropdownMenuItem>
          {[
            "title",
            ...
            <DropdownMenuItem
              key={fieldId}
              onClick={() =>
                table.tableSort.setSort([{ fieldId, direction: "asc" }])
              }
            >
              {fieldId}
            </DropdownMenuItem>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="start" className="w-48">
          {["list", "status", "priority", "assignee"].map((groupBy) => (
            <DropdownMenuItem
              key={groupBy}
              onClick={() => table.updateViewConfig({ groupBy })}
            >
              {groupBy}
            </DropdownMenuItem>
          ))}
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem
            onClick={() => table.updateViewConfig(defaultTableViewConfig)}
          >
            Reset table view
          </DropdownMenuItem>
        </DropdownMenuContent>
    ...
    <div className="p-4">
      <div className="rounded-lg border border-border bg-card p-8 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          The board table could not be loaded.
        </p>
      </div>
    </div>
  );
