...
import { useUpdateCard, useUpdateFieldValue } from "@notrelix/work-management-state"
import type { Board, BoardMember, Card, FieldDefinition, FieldOption } from "@notrelix/work-management-core"
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <button type="button" className="flex max-w-full items-center gap-2 text-left" onClick={(event: any) => event.stopPropagation()}>
      ...
      <PopoverContent align="start" className="w-64 p-2" onClick={(event: any) => event.stopPropagation()}>
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button type="button" className="max-w-full text-left" onClick={(event: any) => event.stopPropagation()} aria-label={`${field.name}: ${option?.label ?? "Empty"}`}>
          {option ? (
            <Badge
              variant="secondary"
      ...
      <DropdownMenuContent align="start" onClick={(event: any) => event.stopPropagation()}>
        {options.map((item) => (
...
function DateCell({ card, field }: { card: Card; field: FieldDefinition }) {
  ...
  return (
    <Popover>
      ...
      <PopoverContent align="start" className="w-auto p-0" onClick={(event: any) => event.stopPropagation()}>
...
function TimelineCell({ card, field }: { card: Card; field: FieldDefinition }) {
  ...
  return (
    <div className="flex min-w-0 items-center gap-1" onClick={(event: any) => event.stopPropagation()}>
      <Input
        type="date"
        value={toDateInputValue(value.start)}
        onChange={(event: any) => updateFieldValue.mutate({ cardId: card.id, fieldDefinitionId: field.id, value: { ...value, start: event.target.value } })}
      ...
      <Input
        ...
        onChange={(event: any) => updateFieldValue.mutate({ cardId: card.id, fieldDefinitionId: field.id, value: { ...value, end: event.target.value } })}
        className="h-8 w-[120px] bg-card"
...
function TextFieldCell({ card, field }: { card: Card; field: FieldDefinition }) {
  ...
  return (
    <button type="button" className="min-w-0 truncate text-left text-sm text-foreground/80 hover:text-foreground transition-colors" onClick={(event: any) => {
      event.stopPropagation()
      setEditing(true)
    }}>
