...
export function getChecklistProgress(card: Card) {
  const total = card.checklists.reduce((count: any, checklist: any) => count + checklist.items.length, 0)
  const done = card.checklists.reduce((count: any, checklist: any) => count + checklist.items.filter((item: any) => item.isDone).length, 0)
