...
export function BoardCalendarView({
  ...
  const cardsByDay = useMemo(() => {
    const result: Record<number, CalendarCard[]> = {
      0: [],
      1: [],
      2: [],
      3: [],
      ...
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Unified deadlines from board cards and linked docs.
          </p>
        </div>
      ...
      <div className="grid min-w-[760px] grid-cols-5">
        {currentWeekDays.map((day, index) => {
          ...
          return (
            <div
              key={day.toISOString()}
              className="min-h-[520px] border-r border-border p-3 last:border-r-0"
            >
