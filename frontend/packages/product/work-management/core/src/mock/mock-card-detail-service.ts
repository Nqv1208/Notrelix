...
const cardUpdates: CardUpdate[] = mockCardComments.map((comment, index) => {
  ...
  const author = memberToCardMember(
    board.board.members[index % board.board.members.length]!,
    comment.cardId,
  );
  ...
  board.groups.flatMap((group) =>
    group.cards.flatMap((card) =>
      Array.from({ length: card._count.attachments }).map((_, index) => ({
        ...
        cardId: card.id,
        name:
          index === 0 ? "product-spec.pdf" : `workspace-asset-${index + 1}.png`,
...
export const mockCardDetailService = {
  ...
  async getCardDetail(cardId: string): Promise<CardDetail> {
    ...
    return {
      ...
      watchers: board.board.members
        .slice(0, 3)
        .map((member) => memberToCardMember(member, cardId)),
      isWatched: true,
  ...
  async getCardUpdates(cardId: string): Promise<CardUpdate[]> {
    ...
    return cardUpdates
      ...
      .map((update) => ({
        ...
        mentionUserIds: [...update.mentionUserIds],
