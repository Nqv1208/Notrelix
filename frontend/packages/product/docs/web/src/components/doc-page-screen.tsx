...
const BLOCK_MENU_ITEMS: Array<{ type: BlockType; label: string; icon: React.ReactNode; keywords: string[] }> = [
  { type: 'paragraph', label: 'Text', icon: <AlignLeft className="h-4 w-4" />, keywords: ['text', 'paragraph', 'plain'] },
  { type: 'heading_1', label: 'Heading 1', icon: <Heading1 className="h-4 w-4" />, keywords: ['heading', 'h1', 'title'] },
  { type: 'heading_2', label: 'Heading 2', icon: <Heading2 className="h-4 w-4" />, keywords: ['heading', 'h2', 'subtitle'] },
  { type: 'heading_3', label: 'Heading 3', icon: <Heading3 className="h-4 w-4" />, keywords: ['heading', 'h3'] },
  { type: 'bulleted_list', label: 'Bullet List', icon: <List className="h-4 w-4" />, keywords: ['bullet', 'list', 'unordered'] },
  { type: 'numbered_list', label: 'Numbered List', icon: <ListOrdered className="h-4 w-4" />, keywords: ['number', 'list', 'ordered'] },
  { type: 'todo', label: 'To-do', icon: <CheckSquare className="h-4 w-4" />, keywords: ['todo', 'checkbox', 'task'] },
  { type: 'quote', label: 'Quote', icon: <Quote className="h-4 w-4" />, keywords: ['quote', 'blockquote'] },
  { type: 'divider', label: 'Divider', icon: <Minus className="h-4 w-4" />, keywords: ['divider', 'hr', 'line', 'separator'] },
  { type: 'code', label: 'Code', icon: <Code className="h-4 w-4" />, keywords: ['code', 'pre', 'monospace'] },
  { type: 'callout', label: 'Callout', icon: <span className="text-sm">💡</span>, keywords: ['callout', 'info', 'note'] },
];
...
function SortableBlockEditor({ pageId, block, onDelete, useUpdateBlock }: BlockEditorProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    <div ref={setNodeRef} style={style} className="group flex items-start gap-2.5 relative -ml-6 pl-6">
      ...
      <div className="flex-1 relative">
        <BlockEditorContent pageId={pageId} block={block} useUpdateBlock={useUpdateBlock} />
      </div>
    </div>
  );
}

function BlockEditorContent({ pageId, block, useUpdateBlock }: Omit<BlockEditorProps, 'onDelete'>) {
  ...
  const renderBlockContent = () => {
    switch (block.type) {
      ...
      case 'numbered_list':
        return (
          <div className="flex items-start gap-2">
            <span className="text-muted-foreground text-sm mt-0.5 shrink-0">1.</span>
            <input
              ref={inputRef}
              type="text"
              defaultValue={block.properties.text || ''}
      ...
      case 'callout':
        return (
          <div className="flex items-start gap-3 p-4 rounded-xl border bg-muted/30">
            <span className="text-xl shrink-0">{block.properties.icon || '💡'}</span>
            <input
              ref={inputRef}
              type="text"
              defaultValue={block.properties.text || ''}
...
function FormattingToolbar({ onFormat }: { onFormat: (format: string) => void }) {
  const tools = [
    { icon: <Bold className="h-3.5 w-3.5" />, format: 'bold', label: 'Bold' },
    { icon: <Italic className="h-3.5 w-3.5" />, format: 'italic', label: 'Italic' },
    { icon: <Underline className="h-3.5 w-3.5" />, format: 'underline', label: 'Underline' },
    { icon: <Strikethrough className="h-3.5 w-3.5" />, format: 'strikethrough', label: 'Strikethrough' },
  ];
...
export function createDocPageScreen({ api, endpoints }: CreateDocPageScreenDeps) {
  ...
  return function DocPageScreen({ workspaceId, pageId }: { workspaceId: string; pageId: string }) {
    ...
    const { data: blocks = [], isLoading: blocksLoading } = usePageBlocks(pageId) as { data: Block[]; isLoading: boolean };
    ...
    const [sidePanel, setSidePanel] = useState<'comments' | 'history' | 'tree' | null>(null);
        ...
        <div className="flex-1 overflow-y-auto flex flex-col h-full">
          ...
          <div className="px-8 py-2 flex items-center justify-between border-b">
            <div className="flex items-center gap-1">
              <Button
                ...
                className={`h-7 gap-1.5 text-xs ${
                  page.isFavorited ? 'text-yellow-500' : 'text-muted-foreground hover:text-foreground'
                }`}
              >
                <span className={page.isFavorited ? 'fill-yellow-500' : ''}>★</span>
            ...
            <div className="flex items-center gap-1">
              <Button
                ...
                size="icon"
                className="h-7 w-7 text-muted-foreground hover:text-foreground"
                onClick={() => setSidePanel(sidePanel === 'tree' ? null : 'tree')}
              >
          ...
          <div className="flex-1 max-w-3xl w-full mx-auto px-8 sm:px-12 py-10 space-y-6">
            ...
            <div className="space-y-3">
              {blocks.length === 0 ? (
                <p className="text-sm text-muted-foreground italic py-4">
                  Press <kbd className="px-1.5 py-0.5 bg-muted rounded text-xs font-mono">/</kbd> for commands or type to start writing.
                </p>
              ) : (
            ...
            <div className="border-t pt-4 flex flex-wrap gap-1.5">
              {BLOCK_MENU_ITEMS.filter((item) => ['paragraph', 'heading_1', 'heading_2', 'todo', 'bulleted_list', 'quote', 'divider'].includes(item.type)).map((item) => (
                <Button
                  key={item.type}
                  variant="ghost"
                  size="sm"
