...
function validateBlockContent(blockType: 'paragraph' | 'heading' | 'code' | 'to_do', content: string): boolean {
...
describe('Docs Core Invariants', () => {
  ...
  it('docsQueryKeys should format page detail key', () => {
    expect(docsQueryKeys.detail('page-777')).toEqual(['pages', 'detail', 'page-777']);
  });
});
