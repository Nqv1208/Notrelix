...
import type { AutomationExecution, AutomationExecutionStep } from '@notrelix/automation-core';
...
export function isAutomationExecutionEventType(
  ...
  return automationExecutionEventTypes.includes(eventType as AutomationExecutionEventType);
}
...
export function createAutomationExecutionRealtimeAdapter(
  ...
  return {
    ...
    async validateAndHandle(
      ...
      if (!payload) {
        await context.invalidateQueries([[...automationQueryKeys.workspace(context.workspaceId)]]);
        return;
      }

      const detailKey = automationQueryKeys.executionDetail(payload.executionId);
      const existing = queryClient.getQueryData<AutomationExecution>(detailKey);
      if (existing && isOlderOrEqual(payload, existing)) {
        return;
      }

      queryClient.setQueryData<AutomationExecution | undefined>(detailKey, (current) =>
        reconcileExecutionEvent(current, envelope, payload)
      );

      await context.invalidateQueries([
        [...automationQueryKeys.executionHistory(context.workspaceId, payload.ruleId)],
      ]);
    },
  };
}
...
function parseAutomationExecutionPayload(
  ...
  if (typeof candidate.executionId !== 'string' || typeof candidate.ruleId !== 'string') {
    return null;
  }
  return {
...
function reconcileExecutionEvent(
  ...
  return {
    ...
    completedAt: isTerminalStatus(status) ? envelope.timestamp : current?.completedAt,
    error: payload.error ?? current?.error,
    steps: payload.step ? upsertStep(previousSteps, payload.step) : previousSteps,
