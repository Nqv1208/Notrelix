...
export function createInvitationsService(
  ...
  return {
    ...
    async create(
      ...
      if (mockMode) {
        console.warn(
          "POST /workspaces/{workspaceId}/invitations is pending backend validation.",
        );
        return {
          id: "stub-invitation",
          email: _input.email,
    ...
    async delete(_workspaceId: string, _invitationId: string): Promise<void> {
      if (mockMode) {
        console.warn(
          "DELETE /workspaces/{workspaceId}/invitations/{invitationId} is pending backend validation.",
        );
        return;
      }
      throw new Error("Endpoint not implemented by backend.");
