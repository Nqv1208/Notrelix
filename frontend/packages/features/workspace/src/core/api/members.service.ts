...
export function createMembersService(
  ...
  return {
    ...
    async updateRole(
      ...
      if (mockMode) {
        console.warn(
          "PUT /workspaces/{workspaceId}/members/{userId}/role is pending backend validation.",
        );
    ...
    async removeMember(_workspaceId: string, _userId: string): Promise<void> {
      if (mockMode) {
        console.warn(
          "DELETE /workspaces/{workspaceId}/members/{userId} is pending backend validation.",
        );
        return;
      }
      throw new Error("Endpoint not implemented by backend.");
