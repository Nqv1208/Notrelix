...
export function ActiveBoards({ workspaceId, boards, isLoading }: ActiveBoardsProps) {
  return (
    <div className="rounded-xl border border-border/60 bg-card/50 p-5">
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-muted-foreground py-4">No boards yet. Create your first board to get started.</p>
      ) : (
        <div className="space-y-2">
          {boards.slice(0, 4).map((board, i) => (
            ...
            >
              <div className={cn('size-1 rounded-full mt-2 shrink-0', accentColors[i % accentColors.length])} />
              <div className="flex-1 min-w-0">
                <p className="font-medium text-sm truncate group-hover:text-foreground text-foreground">{board.title}</p>
                {board.description && (
