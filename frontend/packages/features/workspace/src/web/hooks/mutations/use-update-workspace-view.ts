...
export function createUseUpdateWorkspaceView({ api }: UseUpdateWorkspaceViewDeps) {
  ...
  return function useUpdateWorkspaceView(workspaceId: string, viewId: string) {
    ...
    return useMutation({
      mutationFn: (input: UpdateWorkspaceViewInput) => service.update(workspaceId, viewId, input),
      onSuccess: () => {
