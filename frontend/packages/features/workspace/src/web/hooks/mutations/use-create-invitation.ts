...
export function createUseCreateInvitation({ api, endpoints }: UseCreateInvitationDeps) {
  ...
  return function useCreateInvitation(workspaceId: string) {
    ...
    return useMutation({
      mutationFn: (input: CreateWorkspaceInvitationInput) => service.create(workspaceId, input),
      onSuccess: () => {
