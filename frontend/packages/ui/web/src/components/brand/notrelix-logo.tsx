...
type NotrelixLogoSize = "sm" | "md" | "lg";
...
interface NotrelixLogoArtworkProps {
...
export function NotrelixLogoMark({
  ...
  return (
    <svg
      ...
      {...props}
    >
      <NotrelixLogoArtwork gradientIdPrefix="notrelix" />
    </svg>
  );
}
...
const BRAND_LOADER_REVEAL_DELAY_MS = 250;

export function NotrelixBrandLoader({
  size = "md",
  className,
  "aria-label": ariaLabel = "Loading Notrelix",
  ...props
}: NotrelixBrandLoaderProps) {
  const [isVisible, setIsVisible] = React.useState(false);
  const reactId = React.useId();
  const instanceId = reactId.replace(/[^a-zA-Z0-9_-]/g, "");
  const gradientIdPrefix = `notrelix_loader_${instanceId}`;
  const maskId = `${gradientIdPrefix}_mask`;

  React.useEffect(() => {
    const timeoutId = window.setTimeout(
      () => setIsVisible(true),
      BRAND_LOADER_REVEAL_DELAY_MS,
    );
  ...
  return (
    <div
      ...
      data-slot="notrelix-brand-loader"
    ...
    >
      ...
      >
        <defs>
          ...
          >
            <path
              d="M36 296 L82 104 L321 230 L380 20"
              pathLength="1"
              fill="none"
              stroke="white"
              strokeWidth="118"
        ...
        <g mask={`url(#${maskId})`}>
          <NotrelixLogoArtwork gradientIdPrefix={gradientIdPrefix} />
          <animate
      ...
      <NotrelixLogoMark
        aria-hidden="true"
        className={cn(
          "hidden shrink-0 motion-reduce:block",
          loaderSizeClasses[size],
        )}
      />
    </div>
