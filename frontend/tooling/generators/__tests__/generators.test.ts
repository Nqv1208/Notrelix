...
function runGenerator(generator: string, ...args: string[]): void {
  execFileSync(
    process.execPath,
    [join(generatorsDir, generator, "index.mjs"), ...args],
    {
...
function runVitest(include: string[]): void {
  ...
  execFileSync(
    bin("vitest"),
    ["run", "--config", join(tempDir, "vitest.config.mjs")],
    {
      cwd: tempDir,
      stdio: "inherit",
    },
...
function runArchitectureCheck(): void {
  ...
  execFileSync(
    tsx,
    [
      join(tempDir, "tooling/dependency-rules/src/run-checks.ts"),
      "--root",
      tempDir,
    ],
    { stdio: "inherit" },
...
describe("create-feature generator", () => {
  ...
  it("rejects an invalid --ui target", () => {
    expect(() =>
      runGenerator("create-feature", "billing", "--ui", "desktop"),
    ).toThrow(/Invalid --ui target/);
  });
...
describe("create-ui-component generator", () => {
  it("scaffolds a web component with export, test, and story", () => {
    ...
    const componentFile = join(
      tempDir,
      "packages/ui/web/src/components/ui/alert.tsx",
    );
    ...
    expect(
      existsSync(
        join(tempDir, "tooling/storybook/web/stories/alert.stories.tsx"),
      ),
    ).toBe(true);

    const index = readFileSync(
      join(tempDir, "packages/ui/web/src/index.ts"),
      "utf8",
    );
  ...
  it("scaffolds a mobile contract with export and test", () => {
    ...
    const index = readFileSync(
      join(tempDir, "packages/ui/mobile/src/index.ts"),
      "utf8",
    );
  ...
  it("rejects an invalid --target", () => {
    expect(() =>
      runGenerator("create-ui-component", "alert", "--target", "server"),
  ...
  it(
    ...
    () => {
      ...
      expect(fixtureManifest).toContain(
        "packageName: '@notrelix/features-freeze-smoke-feature'",
      );
      expect(fixtureManifest).toContain(
        "relativePath: 'packages/features/freeze-smoke-feature'",
      );
      expect(fixtureManifest).toContain(
      ...
      const featureDir = join(
        tempDir,
        "packages/features/freeze-smoke-feature",
      );
