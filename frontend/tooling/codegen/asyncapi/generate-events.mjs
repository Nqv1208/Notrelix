...
if (!existsSync(specPath)) {
  console.error('Required Realtime AsyncAPI spec missing: artifacts/contracts/realtime.v1.json');
  process.exit(1);
}
...
try {
  ...
  for (const messageKey of stableKeys(messages)) {
    ...
    lines.push(`export interface ${payloadName} ${schemaToTs(message.payload, 0)}`);
    lines.push('');
    unionMembers.push(`  | { eventType: ${JSON.stringify(eventType)}; payload: ${payloadName} }`);
  }

  lines.push('export type GeneratedRealtimeMessage =');
  lines.push(unionMembers.length > 0 ? unionMembers.join('\n') + ';' : '  never;');
  lines.push('');
  ...
  console.log('Generated realtime messages from artifacts/contracts/realtime.v1.json');
} catch (err) {
