...
export function checkPackageManifests(rootDir: string): {
  ok: boolean;
  violations: string[];
} {
  ...
  for (const pkg of discovered) {
    ...
    for (const dep of declaredDeps) {
      if (forbidden.includes(dep)) {
        violations.push(
          `[DECLARED_FORBIDDEN_DEPENDENCY] ${pkg.name} declared forbidden dependency "${dep}"`,
        );
      }
      ...
      if (internalPkg.startsWith("@notrelix/") && !isTooling) {
        ...
        if (
          ...
          violations.push(
            `[DECLARED_INTERNAL_DEP_NOT_ALLOWED] ${pkg.name} declared disallowed dependency "${internalPkg}"`,
          );
        }
      }
    }
