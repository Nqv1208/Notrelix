...
test("renders the configured deterministic scenario", async ({ page }) => {
  test.skip(!["new-user", "empty", "error", "large", "permissions"].includes(scenario), "non-default scenario only");
  await page.goto("/home");
  if (scenario === "new-user") {
    await expect(page.getByText("No workspaces are available for this account.")).toBeVisible();
