...
test("loads auth and workspace routes without backend network", async ({ page }) => {
  test.skip((process.env.VITE_MOCK_SCENARIO ?? "default") !== "default", "default scenario only");
  const backendRequests: string[] = [];
  const backendSockets: string[] = [];
  page.on("request", (request) => {
    if (request.url().includes("127.0.0.1:59999")) backendRequests.push(request.url());
  });
  page.on("websocket", (socket) => {
    if (socket.url().includes("127.0.0.1:59998")) backendSockets.push(socket.url());
  });
  ...
  await expect(page.getByRole("heading", { name: "Recent docs" })).toBeVisible();
  ...
  await expect(page.getByRole("menuitem", { name: "My profile" })).toBeEnabled();
  await expect(page.getByRole("menuitem", { name: "Marketplace" })).toBeDisabled();
  await page.getByRole("menuitem", { name: "My profile" }).click();
  await expect(page).toHaveURL(/\/workspaces\/mock-workspace-primary\/account\/profile$/);
  ...
  await expect(page.getByText("Search workspace", { exact: true })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "Active Boards" })).toBeVisible();
