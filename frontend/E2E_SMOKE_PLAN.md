...
test("Shows Mock Disabled state when mock mode is disabled", async ({
  page,
}) => {
  ...
  await expect(
    page.locator("text=Tính năng đang được phát triển"),
  ).toBeVisible();
...
test("Shows Access Denied state for unauthorized resources", async ({
  page,
}) => {
  ...
  await expect(
    page.locator("text=Tài khoản của bạn không được cấp quyền"),
  ).toBeVisible();
});
```
