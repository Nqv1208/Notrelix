...
import { isDocsMockModeEnabled } from "../../shared/mock/docs-mock-mode"
import type { Page as LegacyPage } from "../../shared/types/document.types"
...
export function DocsDashboardPreview() {
  const router = useRouter()
  
  if (!isDocsMockModeEnabled()) {
    return (
      <div className="max-w-5xl mx-auto px-6 py-10 text-center">
        <h1 className="text-3xl font-bold tracking-tight">Dashboard</h1>
        <p className="text-muted-foreground mt-2">
          Mock mode is disabled. Connect to the production API to view live dashboard data.
        </p>
      </div>
    )
  }
  ...
  const allPages: (LegacyPage & { wsName: string; wsIcon: string })[] = []
...
function flattenPages(pages: LegacyPage[]): LegacyPage[] {
  const result: LegacyPage[] = []
