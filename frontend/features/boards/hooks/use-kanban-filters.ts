  ...
  const filteredGroups = useMemo(() => {
    ...
    return groups.map((group) => ({
      ...
      cards: group.cards.filter((card) => {
        ...
        if (filters.priority.length > 0 && (!card.priority || !filters.priority.includes(card.priority))) {
          return false
        }
        if (filters.assigneeId.length > 0) {
          const cardUserIds = card.members.map((m) => m.userId)
          const matches = filters.assigneeId.some((id) => cardUserIds.includes(id))
          if (!matches) return false
        }
        if (filters.labelId.length > 0) {
          const cardLabelIds = card.labels.map((l) => l.id)
          const matches = filters.labelId.some((id) => cardLabelIds.includes(id))
          if (!matches) return false
        }
        return true
      }),
