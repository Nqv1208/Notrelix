...
public class ShareLinkBoundaryTests
{
    ...
    public void ShareLink_Create_WithPublicAccessAndNoExpiry_ShouldThrow()
    {
        ...
        var act = () => ShareLink.Create(Guid.NewGuid(), WsA, ResourceKind.Create("work-management.board"), Guid.NewGuid(), tokenHash, ShareLinkAccessMode.Public, Actor, Now);
    ...
    {
        var tokenHash = ShareLinkTokenHash.Create("test-hash");
        var link = ShareLink.Create(Guid.NewGuid(), WsA, ResourceKind.Create("work-management.board"), Guid.NewGuid(), tokenHash, ShareLinkAccessMode.Public, Actor, Now, Now.AddDays(7));
    ...
    {
        var tokenHash = ShareLinkTokenHash.Create("test-hash");
        var link = ShareLink.Create(Guid.NewGuid(), WsA, ResourceKind.Create("work-management.board"), Guid.NewGuid(), tokenHash, ShareLinkAccessMode.WorkspaceOnly, Actor, Now);
        link.Should().NotBeNull();
    ...
    {
        var tokenHash = ShareLinkTokenHash.Create("test-hash");
        var link = ShareLink.Create(Guid.NewGuid(), WsA, ResourceKind.Create("work-management.board"), Guid.NewGuid(), tokenHash, ShareLinkAccessMode.WorkspaceOnly, Actor, Now, Now.AddDays(-1));
        link.IsExpired(Now).Should().BeTrue();
    }
}
