...
public static class TeamLeadRules
{
    public static void EnsureCanRemoveLead(int activeLeadCount)
    {
        if (activeLeadCount <= 1)
            throw new BusinessRuleException(
                BusinessRuleCodes.Workspaces_Team_CannotRemoveLastLead,
        ...
        int activeLeadCount)
    {
        ...
        {
            throw new BusinessRuleException(
                BusinessRuleCodes.Workspaces_Team_CannotDowngradeLastLead,
        ...
        int activeLeadCount)
    {
        if (role == TeamMemberRole.Lead && activeLeadCount <= 1)
            throw new BusinessRuleException(
                BusinessRuleCodes.Workspaces_Team_LastLeadCannotLeave,
