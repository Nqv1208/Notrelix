...
public sealed class KanbanViewConfig : BoardViewConfig
{
        ...
        Guid? swimlaneFieldId = null)
    {
        ...
        if (!columnField.CanBeUsedAsKanbanColumn())
            throw new BusinessRuleException(WorkManagementRuleCodes.WorkManagement_Kanban_InvalidColumnField, "Invalid Kanban column field.");

        var ids = visibleFieldIds.ToArray();
        if (ids.Length == 0)
            throw new BusinessRuleException(WorkManagementRuleCodes.WorkManagement_Kanban_MustHaveVisibleField, "Kanban view must have at least one visible field.");
        if (ids.Any(id => id == Guid.Empty))
            throw new BusinessRuleException(WorkManagementRuleCodes.WorkManagement_Kanban_VisibleFieldIdsCannotBeEmpty, "Visible field IDs cannot be empty.");

        if (swimlaneFieldId.HasValue && swimlaneFieldId.Value == Guid.Empty)
            throw new BusinessRuleException(WorkManagementRuleCodes.WorkManagement_Kanban_SwimlaneFieldIdCannotBeEmpty, "Swimlane field ID cannot be empty.");
