...
public sealed class FormQuestionConfig : ValueObject
{
    ...
    private static void ValidateConfig(FormQuestionType type, int? maxLength, int? minValue, int? maxValue, int? maxFileSizeMb)
    {
        if (maxLength.HasValue && type != FormQuestionType.ShortText && type != FormQuestionType.LongText)
            throw new BusinessRuleException($"MaxLength is only valid for ShortText and LongText question types, not {type}.");

        if ((minValue.HasValue || maxValue.HasValue) && type != FormQuestionType.Number)
            throw new BusinessRuleException($"MinValue/MaxValue are only valid for Number question type, not {type}.");

        if (maxFileSizeMb.HasValue && type != FormQuestionType.FileUpload)
            throw new BusinessRuleException($"MaxFileSizeMb is only valid for FileUpload question type, not {type}.");

        if (maxLength.HasValue && maxLength.Value <= 0)
            throw new BusinessRuleException("MaxLength must be positive.");

        if (maxFileSizeMb.HasValue && maxFileSizeMb.Value <= 0)
            throw new BusinessRuleException("MaxFileSizeMb must be positive.");

        if (minValue.HasValue && maxValue.HasValue && minValue.Value > maxValue.Value)
            throw new BusinessRuleException("MinValue cannot be greater than MaxValue.");
    }
