...
-- audit.activity_logs (legacy compatibility)
ALTER TABLE audit.activity_logs ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS p_audit_activity_logs_insert_app ON audit.activity_logs;
CREATE POLICY p_audit_activity_logs_insert_app ON audit.activity_logs
    FOR INSERT TO notrelix_app, notrelix_worker
    WITH CHECK (
        workspace_id IS NULL
        OR workspace_id = ops.current_workspace_id()
    );

DROP POLICY IF EXISTS p_audit_activity_logs_worker_all ON audit.activity_logs;
CREATE POLICY p_audit_activity_logs_worker_all ON audit.activity_logs
    FOR ALL TO notrelix_worker
    USING (true) WITH CHECK (true);
