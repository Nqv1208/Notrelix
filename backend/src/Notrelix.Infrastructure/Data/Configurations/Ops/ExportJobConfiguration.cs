...
public class ExportJobConfiguration : IEntityTypeConfiguration<ExportJobRecord>
{
    public void Configure(EntityTypeBuilder<ExportJobRecord> builder)
    {
        ...
        builder.Property(x => x.Format).HasColumnName("format").IsRequired().HasMaxLength(40).HasDefaultValue("Csv");
        ...
        builder.Property(x => x.FiltersJson).HasColumnName("filters_json").HasColumnType("jsonb").HasDefaultValue("{}");
        ...
        builder.Property(x => x.DownloadUrl).HasColumnName("download_url");
        ...
        builder.HasIndex(x => new { x.WorkspaceId, x.Status, x.CreatedAt }).IsDescending(false, false, true).HasDatabaseName("ix_ops_export_jobs_workspace_status");
    }
}
