    ...
    public void Configure(EntityTypeBuilder<EmailOutboxMessage> builder)
    {
        ...
        builder.ToTable(table =>
        {
            table.HasCheckConstraint(
                ...
                "(content_mode = 'Rendered' AND subject IS NOT NULL AND (body_html IS NOT NULL OR body_text IS NOT NULL) AND template_data_json IS NULL)" +
                " OR (content_mode = 'Templated' AND subject IS NULL AND body_html IS NULL AND body_text IS NULL AND template_data_json IS NOT NULL AND template_data_json <> '{}'::jsonb)" +
                " OR (content_mode = 'Purged' AND subject IS NULL AND body_html IS NULL AND body_text IS NULL AND template_data_json IS NULL AND sensitive_payload_cleared_at IS NOT NULL)");
            table.HasCheckConstraint(
