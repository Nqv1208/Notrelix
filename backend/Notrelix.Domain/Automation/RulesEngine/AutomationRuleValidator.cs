...
public static class AutomationRuleValidator
{
    public static void Validate(Rules.AutomationRule rule)
    {
        ...
        if (rule.Configuration is null)
            throw new BusinessRuleException("Automation rule must have a configuration with trigger and action.");

        if (rule.Configuration.Trigger is null)
            throw new BusinessRuleException("Automation rule configuration must have a trigger.");

        if (rule.Configuration.Action is null)
            throw new BusinessRuleException("Automation rule configuration must have an action.");
