...
public class ArchitectureTests
{
    private static string GetApiPath()
    {
        ...
        while (current != null && !Directory.Exists(Path.Combine(current, "Notrelix.API")))
        {
            current = Path.GetDirectoryName(current);
        }
        if (current == null)
        {
            var fallback = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "../../../../../Notrelix.API"));
            if (Directory.Exists(fallback)) return fallback;
            throw new DirectoryNotFoundException("Could not find Notrelix.API root folder.");
        }
        return Path.Combine(current, "Notrelix.API");
    }
    ...
    public void MapFiles_ShouldContain_RouteComposition()
    {
        ...
        foreach (var file in mapFiles)
        {
            ...
            var hasMapMethod = content.Contains(".Map") && (content.Contains("MapGet") || content.Contains("MapPost") || content.Contains("MapPut") || content.Contains("MapDelete") || content.Contains("MapPatch"));
