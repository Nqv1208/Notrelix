...
public class Phase5AuditTests
{
    ...
    public void Entitlement_Create_WithNegativeLimit_ShouldThrow()
    {
        ...
        var act = () => Entitlement.Create(WsA, feature, -1, EntitlementSource.Subscription);
    ...
    {
        var feature = FeatureCode.Create("boards");
        var entitlement = Entitlement.Create(WsA, feature, 0, EntitlementSource.Subscription);
    ...
    {
        var usage = WorkspaceFeatureUsage.Create(WsA, FeatureCode.Create("storage"), 0, 100, null);
    ...
    {
        var usage = WorkspaceFeatureUsage.Create(WsA, FeatureCode.Create("storage"), 50, 100, null);
    ...
    {
        var act = () => WorkspaceFeatureUsage.Create(WsA, FeatureCode.Create("storage"), -1, 100, null);
        act.Should().Throw<BusinessRuleException>().WithMessage("*negative*");
    }
    ...
    {
        var act = () => WorkspaceFeatureUsage.Create(WsA, FeatureCode.Create("storage"), 0, -1, null);
        act.Should().Throw<BusinessRuleException>().WithMessage("*negative*");
    }
    ...
    {
        var act = () => WorkspaceFeatureUsage.Create(WsA, FeatureCode.Create("storage"), 0, 100, -1);
        act.Should().Throw<BusinessRuleException>().WithMessage("*negative*");
    }
    ...
    {
        var act = () => WorkspaceFeatureUsage.Create(WsA, FeatureCode.Create("storage"), 0, 100, 150);
    ...
    {
        var act = () => WorkspaceFeatureUsage.Create(WsA, FeatureCode.Create("storage"), 150, 100, null, overageAllowed: false);
    ...
    {
        var usage = WorkspaceFeatureUsage.Create(WsA, FeatureCode.Create("storage"), 150, 100, null, overageAllowed: true);
