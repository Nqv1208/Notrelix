...
public record MoveCardCommand(Guid BoardItemId, Guid GroupId, double Position) : ICommand<Result>, ITransactionalRequest;
...
public class MoveCardCommandHandler : IRequestHandler<MoveCardCommand, Result>
{
    ...
    public async Task<Result> Handle(MoveCardCommand request, CancellationToken cancellationToken)
    {
        ...
        if (card.BoardId != targetList.BoardId)
            throw new Notrelix.Domain.Common.Exceptions.BusinessRuleViolationException("CardBoardMismatch", "BoardItem can only be moved between groups on the same board.");
