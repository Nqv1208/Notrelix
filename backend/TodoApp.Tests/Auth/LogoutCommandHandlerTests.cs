...
public class LogoutCommandHandlerTests
{
    ...
    public async Task Handle_WhenSessionExists_ShouldRevokeRefreshToken()
    {
        ...
        var user = TodoApp.Domain.Entities.User.Create("logout@example.com", "Logout User", "hashed");
        context.Users.Add(user);

        var refreshToken = "logout-token";
        var session = TodoApp.Domain.Entities.Session.Create(user.Id, refreshToken, DateTime.UtcNow.AddMinutes(10));
