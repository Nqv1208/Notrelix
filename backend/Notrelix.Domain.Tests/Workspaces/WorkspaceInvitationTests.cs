...
public class WorkspaceInvitationTests
{
    ...
    public void Accept_ShouldSucceed_WhenPendingAndNotExpired()
    {
        ...
        var invitation = WorkspaceInvitation.Create(workspaceId, "test@example.com", WorkspaceRole.Member, "token", Guid.NewGuid());
    ...
    {
        var invitation = WorkspaceInvitation.Create(Guid.NewGuid(), "test@example.com", WorkspaceRole.Member, "token", Guid.NewGuid(), TimeSpan.FromDays(-1));
        
        Action act = () => invitation.Accept(Guid.NewGuid());
