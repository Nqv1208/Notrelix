...
public class ApplicationDbContextInitialiser
{
    ...
    private async Task<List<User>> SeedUsersAsync(int userCount)
    {
        ...
        {
            User.Create("admin@todoapp.com", "Admin User", _passwordHasher.HashPassword("Admin@123")),
            User.Create("demo@todoapp.com", "Demo User", _passwordHasher.HashPassword("Demo@123")),
            User.Create("test@todoapp.com", "Test User", _passwordHasher.HashPassword("Test@123"))
        };

        for (var i = 1; i <= userCount; i++)
        {
            var user = User.Create(
                $"user{i:D3}@todoapp.local",
                $"User {i:D3}",
        ...
        foreach (var workspace in workspaces)
        {
            for (var i = 0; i < invitationsPerWorkspace; i++)
            {
                ...
                invitations.Add(WorkspaceInvitation.Create(
                    workspace.Id,
                    inviter.Id,
                    $"invite-{workspace.Id:N}-{i + 1}@todoapp.local",
                    i % 3 == 0 ? "admin" : "member",
        ...
        foreach (var workspace in workspaces)
        {
            ...
            for (var i = 0; i < attachmentsPerWorkspace; i++)
            {
                ...
                attachments.Add(Attachment.Create(
                    ...
                    user.Id,
                    $"file-{i + 1}.txt",
                    $"https://cdn.todoapp.local/{workspace.Id:N}/file-{i + 1}.txt"));
            }
