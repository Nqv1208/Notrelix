...
public static class ListEndpoints
{
    public static IEndpointRouteBuilder MapListEndpoints(this IEndpointRouteBuilder app)
    {
        // ── Board-scoped routes ──────────────────────────────────
        var boardGroup = app
            .MapGroup("/api/v1/boards/{boardId:guid}/lists")
            .WithTags("Lists")
        ...
        group.MapPatch("/{listId:guid}", UpdateList)
            .WithName("UpdateList")
            .WithSummary("Update list title and color");

        group.MapPost("/{listId:guid}/duplicate", DuplicateList)
            .WithName("DuplicateList")
            .WithSummary("Duplicate a list and its cards");
    ...
    {
        var result = await sender.Send(new CreateListCommand(boardId, body.Title, body.Position, body.Color));
        return result.ToCreatedResult();
    }

    private static async Task<IResult> UpdateList(Guid listId, UpdateListRequest body, ISender sender)
    {
        var result = await sender.Send(new UpdateListCommand(listId, body.Title, body.Color));
        return result.ToApiResult();
    }

    private static async Task<IResult> DuplicateList(Guid listId, ISender sender)
    {
        var result = await sender.Send(new DuplicateListCommand(listId));
        return result.ToCreatedResult();
    }
...
public record CreateListRequest(string Title, double? Position = null, string? Color = null);
public record UpdateListRequest(string? Title = null, string? Color = null);
