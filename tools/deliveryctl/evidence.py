...
def aggregate(plan_path:Path,evidence_dir:Path,output_path:Path,expected_run_id=None):
    plan=json.loads(plan_path.read_text());expected=set(plan.get('ci_expected_proofs') or plan['expected_proofs']);source=plan['source_sha'];planned=plan.get('plan_sha256');records={};problems=[]
    for path in sorted(evidence_dir.rglob('*.json')) if evidence_dir.exists() else []:
        ...
        if planned:
            if not d.get('plan_sha256'):problems.append(f'unbound proof: {proof}')
            elif d.get('plan_sha256')!=planned:problems.append(f'foreign plan proof: {proof}')
    ...
    result={'api_version':'delivery.notrelix.dev/v1','kind':'EvidenceSummary','source_sha':source,'plan_sha256':planned or '','ci_expected_proofs':sorted(expected),'expected_proofs':sorted(expected),'actual_proofs':sorted(actual),'missing':missing,'unexpected':unexpected,'problems':problems,'status':'passed' if not problems else 'failed'}
