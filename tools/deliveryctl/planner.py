...
FRONTEND_FULL_BASELINE={'profile':'frontend-full-baseline','workspace':['node','web','integration','mobile','tooling'],'build':['web','marketing','mobile'],'host_runtime':['web','marketing'],'ui':True,'mock':True}
def _git(root,*args):
...
def build_plan(root=ROOT,event_name='workflow_dispatch',ref='',source_sha='',base_sha='',head_sha='',before_sha='',explicit_changed=None,force_full=False):
    ...
    if full:
        affected.update(all_ids);packages.update(deployables);planes.update({'docs','infra'});security.update({'backend','frontend','runtime'});caps.update(p['defaults']['full_frontend_capabilities']);infra_modes.update(p['infra']['modes']);delivery_platform=True
    elif full_front:
    ...
    if release_candidate:packages=set(deployables);security.update({'backend','frontend','runtime'})
